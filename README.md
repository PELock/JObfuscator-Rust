...
The preferred way of WebApi interface installation is via [crates.io](https://crates.io/).

Run:

```bash
cargo add jobfuscator
```

Or add this entry directly to your `Cargo.toml` in `[dependencies]`:

```toml
jobfuscator = "1.0.0"
```

The crate is available at https://crates.io/crates/jobfuscator

From a checkout of this repository you can run the included examples, for example:

```bash
cargo run --example jobfuscator-example-simple
```
